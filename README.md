...
- Android: 이번 단계는 앱 활성 상태 중심의 단기 수신이며, 전송이 시작된 뒤의 진행률 알림은 foreground service로 표시합니다. 다음 단계에서 gRPC 서버와 SSDP responder 자체를 foreground service 위에서 유지합니다.
...
추가 피쳐 정리와 구현 상태는 [docs/mobile-to-mobile-features.md](docs/mobile-to-mobile-features.md)에 따로 정리했습니다.
