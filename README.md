...
- 모바일 저장소에 `trusted_devices` 테이블 또는 key-value 목록을 둡니다.
- 모바일 서버 롤이 켜져 있을 때 미승인 UUID의 `SendFile`/`ReceiveFile` 요청은 앱 내 확인 팝업이나 foreground notification action으로 승인받습니다.
- Android에서 앱이 백그라운드일 때는 foreground service 알림 action으로 허용/취소를 받고, iOS는 앱 활성 상태 중심으로 처리합니다.

## 모바일-to-모바일과 1:N 전송 검토

모바일-to-모바일은 모바일 앱도 서버 롤을 가져야 하므로 다음 구현 묶음으로 분리합니다.

- Android: 앱이 수신 대기 중일 때 foreground service 위에서 gRPC 서버와 SSDP responder를 유지합니다.
- iOS: 장시간 백그라운드 서버 대기는 제약이 커서 앱이 전면 또는 제한된 백그라운드 시간 안에 있을 때 수신을 우선 지원합니다.
- 1:N 전송: 발견된 여러 서버를 선택하고 같은 파일을 각 서버로 보내는 전송 큐가 필요합니다. PC 앱은 1차 구현을 시작했고, 모바일은 같은 큐 모델을 UI에 이식합니다.
