...
- proto submodule에는 `SubscribeEvents`와 `ListClients` 계약이 포함되어 있습니다.
...
- 실제 연결 구현 단계에서는 `scripts/generate-dart-proto.sh`로 Dart gRPC 코드를 생성한 뒤 이벤트 스트림을 `서버 이벤트` 패널에 연결합니다.
