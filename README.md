...
- 모바일 앱은 PC 서버 주소를 직접 입력해 gRPC `Handshake` 후 `SendFile` client-streaming으로 파일을 보낼 수 있는 코드 경로를 갖습니다.
...
- 아직 SSDP 자동 탐색과 `SubscribeEvents` UI 연결은 다음 단계입니다.
...
1. PC 앱에서 서버를 시작합니다.
2. 모바일 앱의 `PC 서버 주소`에 같은 네트워크의 PC 주소를 `192.168.x.x:39091` 형식으로 입력합니다.
3. Android 에뮬레이터에서 로컬 PC 서버로 보낼 때는 기본값인 `10.0.2.2:39091`을 사용할 수 있습니다.
4. `파일 선택` 후 `파일 보내기`를 누릅니다.
